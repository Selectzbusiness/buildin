      ...
      <div className="p-6 flex flex-col md:flex-row md:items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <h3 className="text-xl font-bold text-[#185a9d] group-hover:text-[#12406a] truncate">{job.title}</h3>
              <div className="text-base font-medium text-gray-700 mt-1 truncate">{companyName}</div>
            </div>
            {/* Company Logo */}
            {companyLogo ? (
              ...
              <div className="w-12 h-12 flex items-center justify-center rounded-lg bg-gray-200 text-[#185a9d] font-bold text-lg ml-2">
                {companyName[0]}
              </div>
            )}
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {/* Experience */}
            {job.experience && (
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-[#185a9d]">
            ...
            {/* Salary */}
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-[#185a9d]">
              <svg className="w-4 h-4 mr-1 text-[#185a9d]" fill="currentColor" viewBox="0 0 20 20">
          ...
          {Array.isArray(job.skills) && job.skills.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {job.skills.map((skill, idx) => (
                <span key={idx} className="px-2 py-0.5 rounded-full bg-[#f4f8fb] text-xs text-[#185a9d] border border-[#b3d4fc] font-medium">
        ...
        <div className="flex flex-col items-end gap-2 min-w-[90px]">
          <button
            onClick={handleSave}
            disabled={saving}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 shadow-sm border border-[#185a9d] ${saved ? 'bg-[#185a9d] text-white' : 'bg-white text-[#185a9d] hover:bg-[#185a9d] hover:text-white'}`}
            aria-label={saved ? 'Unsave job' : 'Save job'}
          >
            <svg className={`w-4 h-4 ${saved ? 'fill-white' : 'fill-[#185a9d] group-hover:fill-white'}`} fill="currentColor" viewBox="0 0 20 20">
